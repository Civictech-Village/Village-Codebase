...
export default function VillageHead({ village }) {
  ...
  return (
        <div style={{ padding: "20px" }}>
            <img className="cover_img" src={village.image} style={{ width: "100%", borderRadius: "6px" }}alt={"img"} />
            <div className="Details bodyboxes" style={{ backgroundColor: "white", padding: "20px", display: "flex", justifyContent: "space-between"}}>
              <div>
                <h1>{village.name}</h1>
                <h5>{village.location}</h5>
              </div>
              <div style={{ padding: "30px" }}>
                <button className="buttn"onClick={handleOpen} style={{ marginRight: "1rem" }}>New Issue</button>
                <button className="buttn" style={{}}>Follow</button>
              </div>
            </div>
            <Modal
        open={open}
